    ...
    try {
        if (connectionString && isProduction) {
            ...
            if (connectionString.startsWith('postgres://') || connectionString.startsWith('postgresql://')) {
                ...
                console.log(`[DB Setup] Attempting to resolve hostname: ${hostname} (Port: ${dbUrl.port})`);
                ...
                try {
                    ...
                    console.error('[DB Setup] DNS resolution failed:', dnsErr.message);
                }
            }
        }
    } catch (err) {
        console.error('[DB Setup] URL parse error, using provided connection string:', err.message);
    }
